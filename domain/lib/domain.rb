...
module DomainGenerator
  ...
  class Domain
    ...
    def initialize(doc, tables=[])
      ...
      @tables = Hash.new
      tables.each { |t| @tables[t.underscore.to_sym] = DomainGenerator::Table.new(doc.root.elements["SQLTable[name='#{t}']"]) }
      
      @tables.each do |k,v|
        v.references[:belongs_to].each { |r| @tables[r].references[:has_many] << k }
#        unless v.references.empty?
#          @tables[k].add_has_many_reference(v.name)
#        end
      end
    end
...
#          # Relationships
#          @references = []
#          if e.elements["referencesTable"]
#            @references << e.elements["referencesTable"].text.pluralize
#          end
#        
...
#    end
    ...
    # Returns fields that should have database indices.
#    def indexed_fields
#      fields.reject { |f| !f.indexed }
#    end
